...
def extract_conv_patches(batch_inputs, kernel_shape):
    ...
    patches = torch.zeros(
        batch_size, c_old, kernel_shape, kernel_shape, h_new, w_new
    ).cuda()

    for i in range(0, h_new):
        for j in range(0, w_new):
            patches[:, :, :, :, i, j] = batch_inputs[
                :, :, i : i + kernel_shape, j : j + kernel_shape
            ]
...
class FixedSizeAttack:
    ...
    def generate_adversarial_example(
        self, model, examples, labels, margins, conv_flag: bool = False
    ):
        kernel_shape = None
        if conv_flag:
        ...
        for i in range(1, 2):
            ...
            raw, clip, _ = self.attack_fn(
                foolbox_model, examples_ep, labels_ep, epsilons=self.epsilons
            )
                    ...
                    for index in range(0, diff_patches.shape[0]):
                        for u in range(0, diff_patches.shape[2]):
                            for v in range(0, diff_patches.shape[3]):
                                temp = torch.linalg.vector_norm(
                                    diff_patches[index, :, :, :, u, v], ord=2
                                ).item()
                                if temp > distances[index]:
                                    distances[index] = temp
                else:
                    distances = self.distance_fn(
                        (clip[k] - examples_ep).reshape((batch_size, 784)), axis=1
                    )

                for j in range(0, batch_size):
                    print(
                        "\n Original margin {}, Current found_adv {}, Adv Classification result {}, adversarial corruption size {}, current best_adv_radius {}".format(
                            margins[j],
                            found_adv[j],
                            adv_class[j],
                            distances[j].item(),
                    ...
                    if margins[j] >= 0:
                        if (adv_class[j] == False) and best_adv_radius[j] > distances[
                            j
                        ].item():
        ...
        best_adv_radius = torch.Tensor(best_adv_radius).to(
...
class MinimizationAttack:
    ...
    def generate_adversarial_example(
        ...
        for i in range(1, 2):
            print("\n Trial number {}".format(i))
            adv_raw, adv_clip, _ = self.attack_fn(
                foolbox_model, examples_ep, labels_ep, epsilons=None
            )
            ...
            if conv_flag:
                ...
                distances = self.distance_fn(
                    (adv_raw - examples_ep).reshape((batch_size, 784)), axis=1
                )
