                ...
                if i == 0:
                    ...
                    if dataset_ == BenchmarkDataset.imagenet:
                        ...
                        state_dict = checkpoint_filter_fn(
                            state_dict, model.rob_model.model
                        )
                        state_dict = add_substr_to_state_dict(state_dict, "model.")
...
def update_json(
    ...
    json_path = (
        Path("model_info") / dataset.value / threat_model_path / f"{model_name}.json"
    )
    ...
    if os.path.exists(json_path):
        with open(json_path, "r") as f:
            ...
            existing_json_dict.update(
                {
                    k: v
                    for k, v in dataclasses.asdict(model_info).items()
                    if v is not None
                }
            )
...
def parse_args():
    ...
    parser.add_argument(
        "--custom_checkpoint", type=str, default="", help="Path to custom checkpoint"
    )
    parser.add_argument(
        "--threat_model",
        type=str,
