        ...
        with torch.no_grad():
            ...
            if state.robust_flags is None:
                ...
                if self.verbose:
                    ...
                    self.logger.log(
                        "robust accuracy at the time of restoring the state: {:.2%}".format(
                            robust_accuracy
                        )
                    )
            ...
            for attack in attacks_to_run:
                ...
                for batch_idx in range(n_batches):
                    ...
                    x_adv[non_robust_lin_idcs] = (
                        adv_curr[false_batch].detach().to(x_adv.device).half()
                    )
                    y_adv[non_robust_lin_idcs] = (
                        output[false_batch].detach().to(x_adv.device).half()
                    )
