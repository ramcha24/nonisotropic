...
def create_standard_eval(
    ...
    def standard_evaluation(model, feedback):
        ...
        output_location = (
            test_output_location + "/" + data_str + "/standard_evaluation/"
        )
        ...
        if verbose:
            ...
            print(
                "Standard Testing : \t loss {:.3f} \t accuracy {:.2f}%\t examples {:d}\t time {:.2f}s".format(
                    test_loss, test_accuracy, int(example_count), elapsed
                )
            )
            print(
                "Target Test Accuracy %.2f at Margin = %.4f"
...
def create_lip_eval(
    ...
    def lipschitz_eval(model, feedback):
        if type_str in registered_estimators.keys():
            registered_estimators[type_str](
                model,
                feedback,
                loader,
                test_output_location,
            ...
            raise ValueError(
                "No registered lipschitz estimator with name {}".format(type_str)
            )

    return lipschitz_eval


def create_attack_eval(
    loader: DataLoader,
    test_output_location,
    data_str,
    random_batch_index,
    ...
    def attack_evaluation(model, feedback):
        ...
        if attack_str == "minimum_size":
            ...
            att = FixedSizeAttack(
                attack, attack_registry.epsilons[attack_str], attack_norm, num_trials
            )
        else:
        ...
        for examples, labels in loader:
            ...
            best_adv, best_adv_radius, found_adv = att.generate_adversarial_example(
                model, examples, labels, margins, conv_flag
            )
...
def create_ensemble_eval(
    ...
    def ensemble_attack_eval(model, feedback):
        ...
        name = "attack_evaluation_" + data_str
        output_location = (
            test_output_location + "/" + data_str + "/ensemble_attack_evaluation/"
        )
        ...
        for examples, labels in loader:
            ...
            minimum_adversary_radius = torch.ones(batch_size).to(
            ...
            clean_adversary_radius = torch.tensor(clean_adversary_radius).to(
                get_platform().torch_device
            )
            ...
            if verbose:
                print(
                    "The ensemble attack evaluation resulted in a robust accuracy of {}".format(
                        robust_accuracy
                    )
                )
                print(
                    "It had a success rate of {} with an average corruption radius {}".format(
                        success_rate, torch.mean(clean_adversary_radius)
                    )
                )

            plot_hist(
                output_location,
                "minimum_adversary_radius",
                "Minimum Radius of Succesful Adversary",
                clean_adversary_radius,
            )
            ...
            feedback[ensemble_key] = {
                "adversarial_examples": best_adversary_overall,
...
def create_security_eval(
    ...
    def security_curve_eval(model, feedback):
        ...
        flaws = 0
        output_location = (
            test_output_location + "/" + data_str + "/security_curve_evaluation/"
        )

        upper = 1.0
        if conv_flag:
            upper = 0.1
        certified_radius_bucket = torch.linspace(0, upper, steps=num_bins_sc).to(
        ...
        certified_accuracy_red_op = torch.zeros_like(certified_radius_bucket).to(
            device=get_platform().torch_device
        )
        certified_accuracy_op_norm = torch.zeros_like(certified_radius_bucket).to(
            device=get_platform().torch_device
        )
        adversarial_accuracy = torch.zeros_like(certified_radius_bucket).to(
            device=get_platform().torch_device
        )
        ...
        for index in range(batch_size):
            ...
            print(
                "\n At index {}, \n (max_red_op_norm, min_adv) is ({}, {})".format(
                    index, red_op_norm_radius[index], minimum_adversary_radius[index]
                )
            )

            if (operator_norm_radius is not None) and red_op_norm_radius[
                index
            ] < operator_norm_radius[index]:
                raise ValueError(
                    "Conflict! why is the reduced op norm certificate {} worse than naive certificate {}?".format(
            ...
            if red_op_norm_radius[index] > minimum_adversary_radius[index]:
                ...
                flaws += 1
                print(
                    "Conflict! there appears to be an adversary of power {} within the radius!! {}".format(
                        minimum_adversary_radius[index], red_op_norm_radius[index]
                    )
                )
                if (operator_norm_radius is not None) and operator_norm_radius[
                    index
                ] < minimum_adversary_radius[index]:
                    red_op_norm_radius[index] = operator_norm_radius[index]
                else:
        ...
        for bin_id in range(num_bins_sc):
            ...
            for index in range(0, batch_size):
                ...
                if (
                    operator_norm_radius is not None
                ) and test_radius <= operator_norm_radius[index]:
            ...
            certified_accuracy_op_norm[bin_id] = (
                certified_accuracy_op_norm[bin_id] / batch_size
            )
            certified_accuracy_red_op[bin_id] = (
                certified_accuracy_red_op[bin_id] / batch_size
            )
        ...
        print(
            "Median reduced op_norm cert radius is {}".format(
                torch.median(red_op_norm_radius).item()
            )
        )
        if operator_norm_radius is not None:
            print(
                "Median op_norm cert radius is {}".format(
                    torch.median(operator_norm_radius).item()
                )
            )
        # print('Median certifiable radius is {}'.format(torch.median(maximum_certifiable_radius).item()))
        print(
            "Median adversary radius is {}".format(
                torch.median(minimum_adversary_radius).item()
            )
        )
        print("total flaws {}".format(flaws))
        plot_security_curve(
            "security_curve_evaluation",
            output_location,
            certified_radius_bucket,
            certified_accuracy_op_norm,
            certified_accuracy_red_op,
...
def evaluation_suite_mnist(
    ...
    if testing_hparams.standard_eval:
        test_standard_eval = create_standard_eval(
            test_set_loader,
            test_output_location,
            "test",
            random_batch_index,
    ...
    if testing_hparams.adv_eval:
        ...
        if attack_type == "best_possible":
            ...
            for (
                attack_name,
                attack,
            ) in attack_registry.fixed_size_attacks_linf.items():
                evaluations.append(
                    create_attack_eval(
                        test_set_loader,
                        test_output_location,
            ...
            raise ValueError(
                "Given adversarial attack type should be one of the following {}".format(
                    possible_attack_str
                )
            )

        evaluations.append(
            create_ensemble_eval(
                test_set_loader,
                test_output_location,
                "test",
                random_batch_index,
        ...
        if conv_flag:
            ...
            evaluations.append(
                create_lip_eval(
                    ...
                    random_batch_index,
                    lip_str,
                    testing_hparams.security_curve_bins,
                    verbose=True,
                )
            )
        else:
            if testing_hparams.lipschitz_estimator_type == "all":
                ...
                testing_hparams.lipschitz_estimator_type in registered_estimators.keys()
            ):
                evaluations.append(
                    create_lip_eval(
                        test_set_loader,
                        ...
                        "test",
                        random_batch_index,
                        testing_hparams.lipschitz_estimator_type,
                        testing_hparams.security_curve_bins,
                        verbose=True,
                    )
                )
    ...
    if (
        testing_hparams.adv_eval
        and testing_hparams.lipschitz_eval
        and testing_hparams.security_eval
    ):
        evaluations.append(
            create_security_eval(
                test_set_loader,
                test_output_location,
                "test",
                random_batch_index,
