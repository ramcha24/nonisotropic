...
def scatter_plot(
    ...
    eq_line = np.linspace(start, end, 100)

    path = os.path.join(output_location, stat_str + ".pdf")
    plt.figure()
    plt.scatter(x, y)
    plt.xlabel(x_label) if x_label is not None else plt.xlabel("x")
    plt.ylabel(y_label) if y_label is not None else plt.ylabel("y")
    plt.title(title) if title is not None else plt.title("Scatter Plot")
    plt.plot(eq_line, eq_line, "r-")
    plt.grid(True)
    plt.savefig(path)
    plt.close()
...
def plot_hist(
    output_location, stat_str, info_str, info_tensor, marker_tensor=None, hist_bins=10
):
    ...
    try:
        ...
        counts, bins, bars = plt.hist(
            info_tensor, bins=hist_bins * 3, range=[0, 500], color="navy"
        )
        plt.xlabel(info_str)
        plt.ylabel("Count")
        # fig, ax = plt.subplots()
...
def plot_encoder(stat_str, output_location, layer_encoder_gaps, layer_ds):
    ...
    for layer_index in range(0, num_layers):
        ...
        # plt.figure()
        ax.plot(
            list(range(0, layer_ds[layer_index])),
            layer_encoder_gaps[layer_index].detach().numpy(),
        )
        ax.set_xlabel("Activity Level at Layer {}".format(layer_index))
        ax.set_ylabel("Encoder Gap")
        if stat_str == "worst":
            ax.set_title(
                "Worst Case Encoder Gap at Layer "
                + str(layer_index)
                + " wrt random batch (128) - MNIST"
            )
        elif stat_str == "average":
            ax.set_title(
                "Average Case Encoder Gap at Layer "
                + str(layer_index)
                + " wrt random batch (128) - MNIST"
            )
        else:
            ax.set_title(
                "Best Case Encoder Gap at Layer "
                + str(layer_index)
                + " wrt random batch (128) - MNIST"
            )
        ax.grid(True)
...
def plot_security_curve(
    stat_str,
    output_location,
    certified_radius_bucket,
    certified_acc_op_norm,
    certified_acc_red_op,
    ...
    plt.semilogx(
        certified_radius_bucket, certified_acc_op_norm, label="patch-global-Lip-cert"
    )
    plt.semilogx(
        certified_radius_bucket, certified_acc_red_op, label="patch-local-Lip-cert"
    )
...
def plot_certified(
    eval_name, stat_str, output_location, c_acc, c_rad, sigmas, c_acc_sp, c_rad_sp
):
    ...
    for k in range(len(c_rad)):
        plt.plot(
            c_rad[k], c_acc[k], "--", label=r"$\sigma$ = %.2f" % np.round(sigmas[k], 2)
        )
    plt.legend(loc="upper right")
...
def generate_layer_angles(layer_matrix, layer_bias, layer_input):
    ...
    for i in range(layer_matrix.shape[0]):
        ...
        cos_theta = (np.dot(row, layer_input) + layer_bias[i]) / (
            row_norm * layer_input_norm
        )
        print(
            "cos theta is {} and angle is {} degrees".format(
                cos_theta, 180 * (np.arccos(cos_theta) / pi)
            )
        )
...
def plot_critical_angle(output_location, model, example):
    ...
    for i, layer in enumerate(model.layers):
        ...
        tan_theta_array = generate_layer_angles(
            model.layers[i].weight.cpu().detach().numpy(),
            model.layers[i].bias.cpu().detach().numpy(),
            layer_input[0].cpu().detach().numpy(),
        )
